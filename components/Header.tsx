'use client';

import { useState, useEffect } from 'react';
import { Menu, X, Phone } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';
import { PHONE_HREF, PHONE_DISPLAY, SITE_NAME } from '@/lib/constants';
import WALink from '@/components/WALink';

const NAV_LINKS = [
  { label: 'Home', href: '/#home' },
  { label: 'Services', href: '/#services' },
  { label: 'Loan Apps', href: '/#loan-apps' },
  { label: 'How It Works', href: '/#how-it-works' },
  { label: 'FAQ', href: '/#faq' },
  { label: 'Blog', href: '/blog' },
  { label: 'Calculator', href: '/tools/loan-penalty-calculator' },
];

const WhatsAppIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4" aria-hidden="true">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
  </svg>
);

const TextLogo = () => (
  <span className="text-xl font-extrabold">
    <span className="text-[#F5A623]">ByeBye</span>
    <span className="text-[#006633]"> Loan</span>
  </span>
);

export default function Header() {
  const [mobileOpen, setMobileOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [logoError, setLogoError] = useState(false);

  useEffect(() => {
    const onScroll = () => setScrolled(window.scrollY > 20);
    window.addEventListener('scroll', onScroll, { passive: true });
    return () => window.removeEventListener('scroll', onScroll);
  }, []);

  const closeMenu = () => setMobileOpen(false);

  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white shadow-md border-b border-gray-100'
          : 'bg-white/95 backdrop-blur-sm border-b border-transparent'
      }`}
    >
      {/* Top announcement bar */}
      <div className="bg-[#006633] text-white text-center text-xs py-2 px-4 font-medium">
        Free Consultation Available — WhatsApp Ya Call Par Rabta Karein ✓
      </div>

      <nav
        className="container mx-auto px-4 sm:px-6 flex items-center justify-between h-20"
        aria-label="Main navigation"
      >
        {/* Logo */}
        <a href="/" className="flex items-center shrink-0" aria-label={SITE_NAME}>
          {logoError ? (
            <TextLogo />
          ) : (
            <Image
              src="/logo.png"
              alt="ByeBye Loan — Pakistan Loan Clearing Service"
              width={200}
              height={56}
              className="h-16 w-auto object-contain drop-shadow-sm"
              priority
              onError={() => setLogoError(true)}
            />
          )}
        </a>

        {/* Desktop nav */}
        <ul className="hidden lg:flex items-center gap-7" role="list">
          {NAV_LINKS.map(({ label, href }) => (
            <li key={href}>
              {!href.includes('#') ? (
                <Link
                  href={href}
                  className="text-sm font-medium text-gray-600 hover:text-[#006633] transition-colors duration-150"
                >
                  {label}
                </Link>
              ) : (
                <a
                  href={href}
                  className="text-sm font-medium text-gray-600 hover:text-[#006633] transition-colors duration-150"
                >
                  {label}
                </a>
              )}
            </li>
          ))}
        </ul>

        {/* Desktop CTAs */}
        <div className="hidden lg:flex items-center gap-3">
          <a
            href={`tel:${PHONE_HREF}`}
            className="flex items-center gap-1.5 text-sm text-[#006633] font-semibold hover:text-[#004d26] transition-colors"
            aria-label={`Call ${PHONE_DISPLAY}`}
          >
            <Phone size={15} aria-hidden="true" />
            {PHONE_DISPLAY}
          </a>
          <WALink
            className="flex items-center gap-2 bg-[#25D366] hover:bg-[#1db954] text-white text-sm font-bold px-4 py-2.5 rounded-xl transition-colors duration-200 shadow"
            aria-label="WhatsApp pe free consultation"
          >
            <WhatsAppIcon />
            WhatsApp Karein
          </WALink>
        </div>

        {/* Mobile hamburger */}
        <button
          className="lg:hidden p-2 text-gray-700 hover:text-[#006633] transition-colors"
          onClick={() => setMobileOpen((v) => !v)}
          aria-label={mobileOpen ? 'Close menu' : 'Open menu'}
          aria-expanded={mobileOpen}
        >
          {mobileOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </nav>

      {/* Mobile dropdown */}
      {mobileOpen && (
        <div className="lg:hidden bg-white border-t border-gray-100 shadow-xl">
          <ul className="container mx-auto px-4 py-4 space-y-1" role="list">
            {NAV_LINKS.map(({ label, href }) => (
              <li key={href}>
                {!href.includes('#') ? (
                  <Link
                    href={href}
                    onClick={closeMenu}
                    className="block py-3 px-3 text-sm font-medium text-gray-700 hover:text-[#006633] hover:bg-[#F0F7F4] rounded-xl transition-colors"
                  >
                    {label}
                  </Link>
                ) : (
                  <a
                    href={href}
                    onClick={closeMenu}
                    className="block py-3 px-3 text-sm font-medium text-gray-700 hover:text-[#006633] hover:bg-[#F0F7F4] rounded-xl transition-colors"
                  >
                    {label}
                  </a>
                )}
              </li>
            ))}
          </ul>
          <div className="container mx-auto px-4 pb-4 grid grid-cols-2 gap-3">
            <a
              href={`tel:${PHONE_HREF}`}
              onClick={closeMenu}
              className="flex items-center justify-center gap-2 border-2 border-[#006633] text-[#006633] font-bold py-3 rounded-xl text-sm transition-colors hover:bg-[#006633] hover:text-white"
            >
              <Phone size={16} aria-hidden="true" />
              Call Karein
            </a>
            <WALink
              onClick={closeMenu}
              className="flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1db954] text-white font-bold py-3 rounded-xl text-sm transition-colors"
            >
              <WhatsAppIcon />
              WhatsApp
            </WALink>
          </div>
        </div>
      )}
    </header>
  );
}
